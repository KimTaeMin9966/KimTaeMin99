...
public class UploadFileUtils {
	...
	/**
	 * 파일 업로드 할때 파일이름 앞에 랜덤한 uuid와 _을 추가하여
	 * uploadPath와 함께 오늘 날짜를 계산하여 그 경로에 파일을 저장하며,
	 * 해당 경로에 올리는 타입이 이미지이면 미리보기 이미지를 만들고 아니면 아이콘을 만들어 해당 경로를 반환한다.
	 * @param originalName
	 * @param uploadPath
	 * @param fileData
	 * @return
	...
	/**
	 * 업로드가 되는 타입이 이미지일 경우 해당 경로의 이미지를 불러와 자동으로 이미지 크기를 줄이고,
	 * 이미지 이름 앞에 s_를 추가 하여 크기를 줄인 이미지를 한번더 저장하고 경로를 반환한다.
	 * @param uploadPath
	 * @param path
	 * @param savedName
	 * @return
